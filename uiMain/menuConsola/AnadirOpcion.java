...
public class AnadirOpcion extends OpcionDeMenu {
	...
	public void ejecutar() {
		...
		while (true) {
			System.out.print(margen + "AÑADIR OPCIÓN\n\n" + (textoError != null ? textoError + "\n\n" : ""));
			...
			if (!Main.listaOpciones.containsKey(opcion = entrada.next()))
				...
				textoError = "El usuario \"" + nombre + "\" ya tiene la opción \"" + opcion + "\".";
			else break;
		}

		Main.usuarios.get(nombre).getMenu().anadirOpcion(Main.listaOpciones.get(opcion));

		System.out.print("\nOpción añadida exitosamente.\n\nPresiona Enter para continuar.");
		esperarEnter();
	}
